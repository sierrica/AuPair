...
@Slf4j
public class TokenProviderOkta implements TokenProvider {

OktaOAuth2Properties oktaOAuth2Properties;
private OktaClientProperties oktaClientProperties;
private static final String OKTAERROR1 = "E0000001";


    public TokenProviderOkta(OktaOAuth2Properties oktaOAuth2Properties, OktaClientProperties oktaClientProperties) {
...
//        	System.out.println("User created with ID: " + user.getEmail());
    		...
    		if (e.getCode().equals(OKTAERROR1) &&
    			e.getMessage().contains("Api validation failed: login") &&
    			e.getCauses().toString().contains("An object with this field already exists in the current organization")) {
    		...
    		else if (e.getCode().equals(OKTAERROR1) &&
        			e.getMessage().contains("Api validation failed: customSecurityQuestion") &&
        			e.getCauses().toString().contains("The security question answer is too weak, answer must not be part of the question")) {
    					throw new InvalidInputException("answerPartQuestion");

        	}
    		else if (e.getCode().equals(OKTAERROR1) &&
        			e.getMessage().contains("Api validation failed: com.saasure.core.services.user.InvalidUserProfileException") &&
        			e.getCauses().toString().contains("Password requirements were not met. Password requirements: at least 8 characters, a lowercase letter, an uppercase letter, a number, no parts of your username")) {
